...
class Edge:
    ...
    def __str__(self):
        return "(" + str(self.x) + "," + str(self.y) + "_D" + str(self.d)
    ...
    def __hash__(self):
        return self.y * 100 + 4 * self.x + self.d
...
edge_pattern = re.compile(r'ln_(\d+)_(\d+)_D(\d)')
...
for var in vars:
    ...
    e[Edge(int(edge.groups()[0]), int(edge.groups()[1]), int(edge.groups()[2]))] = vars[var]
