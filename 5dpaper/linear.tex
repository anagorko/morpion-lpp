...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.592\textwidth]{figures/85_box.pdf}
     \caption{
     A marked Morpion 5D graph $G_{85}$ of size $85$ with its bounding box.
	}
    \label{fig:85}
\end{figure}
...
\begin{figure}
  \centering
    \scalebox{-1}[1]{\includegraphics[width=0.495\textwidth]{figures/94.pdf}}
...
The condition \L{7} enforces that $\ord_{v} \geq \ord_w+1$. %, where $w$ is  any vertex in $m$ that is different from $d(m)$.
  Therefore the algorithm placed $w$ earlier in the gameplay. %, so the required dot $w$ is available. % \L{7} implies that we can place moves in $G$ in order such that a vertex $d(m)$ marked by $m$ 
