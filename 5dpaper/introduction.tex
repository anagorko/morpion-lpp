...
As a single player game, Morpion Solitaire can be  encoded in a natural way as a linear optimization problem with the optimization target being the length of the sequence. The encoding is not very helpful, because the problem is too large to be practically solvable. However, this inspires a natural approach towards construction of upper bounds. Instead of solving the Morpion Solitaire we solve
...
\end{itemize}
We do not expect that the more general game wil be playable by humans. 
\begin{itemize}
\item the %first rigorously proved 
bound of $705$ in Morpion 5T was published in $2006$ \cite[Demaine et al.]{demaine}. The best known bound of $485$ in Morpion 5T was proved in~\cite{ijcai} using an appropriate generalization of Morpion 5T (in fact, the bound of $586$ can be obtained via the relaxation of the original linear program);
\item using another generalization in this paper we prove an upper bound of $84$ on the length of Morpion 5D sequence, improving upon a bound of $121$ found earlier by \cite[Kawamura et al.]{japonczycy}. 
 will significantly improve on the above geometric bounds \cite[Kawamura et al.]{japonczycy}; the method used in  \cite{ijcai} 
%A certain generlization of Morpion Solitaire 5T we analyzed in \cite{ijcai}, however this method 
does not seem very helpful in Morpion 5D --- later in this paper we discuss how these generalizations differ one with another. 
\end{itemize}
...
The plan of the proof is the following. In Theorem \ref{thm:boxes} we show that every Morpion 5D gameplay must be contained in a relatively small box around the starting cross in Figure \ref{fig:initial}.
This already proves the bound of $85$ in Morpion 5D. In Corollary \ref{cor:84} we improve this bound to 84 through an analysis of $4$ special cases. 
%We prove that the
